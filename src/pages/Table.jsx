...
const TimesheetTable = () => {
  ...
  useEffect(() => {
    if (startDate && endDate) {
      ...
      const nextTwoDays = [end.add(1, 'day').format('YYYY-MM-DD'), end.add(2, 'day').format('YYYY-MM-DD')];
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      startDate,
      endDate,
      assignedDefaultHours: 40.00, // Assuming assigned default hours are static
      totalWeeklyWorkedHours: totalWeeklyWorkedHours,
      totalOvertimeWorkedHours: totalWeeklyWorkedHours - 40.00, // Example calculation for overtime
    };

    try {
      const response = await fetch('http://localhost:8080/api/payrollManager/updateWeeklyWorkedHours', {
        method: 'PUT',
        headers: {
  ...
  return (
    <div className="max-w-full mx-auto p-6 border border-gray-300 rounded-lg shadow-md bg-gray-50">
      <h1 className="text-2xl font-semibold text-center mb-6 text-gray-800">Timesheet for {timesheetId}</h1>
      <form onSubmit={handleSubmit}>
        <div className="overflow-x-auto">
          <table className="min-w-full border-collapse">
            <thead>
              <tr>
                <th className="border border-gray-300 px-4 py-2 bg-gray-500 text-white text-left">Timesheet ID</th>
                <th className="border border-gray-300 px-4 py-2 bg-gray-500 text-white text-left">Date</th>
                {dates.map((date) => (
                  <th key={date} className="border border-gray-300 px-4 py-2 bg-gray-500 text-white text-center">
                    {dayjs(date).format('YYYY-MM-DD')}
                  </th>
                ))}
                <th className="border border-gray-300 px-4 py-2 bg-gray-500 text-white text-center">Total Hours</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="border border-gray-300 px-4 py-2 bg-gray-200 text-left">{timesheetId}</td>
                <td className="border border-gray-300 px-4 py-2 bg-gray-200 text-left">Hours Worked</td>
                {dates.map((date) => (
                  <td key={date} className="border border-gray-300 px-4 py-2">
                    <input
                      ...
                      min="0"
                      className={`w-full p-2 border border-gray-300 rounded-md text-gray-900 ${isEditable(date) ? '' : 'bg-gray-200 cursor-not-allowed'}`}
